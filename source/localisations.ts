...
class LocalisationStore {
	...
	buildNameLocalisations({ key }: { key: string }): NameLocalisations | undefined {
		...
		let localisation: Map<Locale, LocalisationBuilder> | undefined;
		...
		const name = localisation?.get(locales.baseLocale)?.();
		if (name === undefined) {
	...
	buildDescriptionLocalisations({ key }: { key: string }): DescriptionLocalisations | undefined {
		...
		const description = localisation?.get(locales.baseLocale)?.({});
	...
	static #toDiscordLocalisations(
		localisations: Map<Locale, (args: Record<string, unknown>) => string>,
	): Localization {
	...
	localiseRaw(key: string, locale?: Locale): RawLocalisationBuilder {
		return (data) => {
			...
			const buildLocalisation =
				localisation.get(locale ?? locales.baseLocale) ?? localisation.get(locales.baseLocale);
	...
	pluralise(key: string, locale: Locale, { quantity }: { quantity: number }): string {
		...
		const pluralised = pluralise(`${quantity}`, { one, two, many }, { locale });
